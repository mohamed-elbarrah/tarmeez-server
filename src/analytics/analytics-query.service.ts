...
export class AnalyticsQueryService {
  ...
  private getPeriodDays(period: string): number {
    return (
      { '1d': 1, '7d': 7, '30d': 30, '90d': 90, '1y': 365, all: 3650 }[
        period
      ] ?? 7
    );
  }
  ...
  async getOverview(storeId: string, period: string) {
    ...
    const avgDuration = dailyRows.length
      ? Math.round(
          dailyRows.reduce((s, r) => s + r.avgDuration, 0) / dailyRows.length,
        )
      : 0;
    ...
    const prevVisitors = prevDailyRows.reduce(
      (s, r) => s + r.uniqueVisitors,
      0,
    );
  ...
  async getTraffic(storeId: string, period: string) {
    ...
    for (const row of rows) {
      const countries = row.countries as Array<{
        country: string;
        count: number;
      }>;
  ...
  async getFunnel(storeId: string, period: string) {
    ...
    const [dailyRows, productViews, cartAdds, checkoutStarts, purchases] =
      await Promise.all([
        this.analyticsPrisma.analyticsDaily.findMany({
          where: { storeId, date: { gte: start } },
