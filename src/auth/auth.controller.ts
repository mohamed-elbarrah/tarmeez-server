...
export class AuthController {
  ...
  async completeRegistration(
    ...
    const result = await this.authService.completeInvitedRegistration(dto);
    this.authService.setTokenCookies(
      res,
      result.accessToken,
      result.refreshToken,
    );
    return { user: result.user, storeName: result.storeName };
  }
  ...
  async customerLogin(
    ...
    this.authService.setCustomerTokenCookies(
      res,
      result.accessToken,
      result.refreshToken,
    );
  ...
  async getMe(@CurrentUser() user: any) {
    if (user?.role === 'CUSTOMER') {
      const profile = await this.authService.getCustomerProfile(
        user.id,
        user.storeId ?? null,
      );
