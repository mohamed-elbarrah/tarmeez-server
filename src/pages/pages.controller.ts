...
export class PagesController {
  ...
  async createPage(@Req() req, @Body() dto: CreatePageDto) {
    return this.pagesService.createPage(dto, req.activeStore.id);
  }

  @Get()
  @Permissions(Resource.PAGES, Action.READ)
  async getPages(@Req() req) {
    return this.pagesService.getPages(req.activeStore.id);
  }
  ...
  async updatePage(@Req() req, @Param('id') id: string, @Body() dto: UpdatePageDto) {
    return this.pagesService.updatePage(id, dto, req.activeStore.id);
  }

  @Patch(':id/status')
  @Permissions(Resource.PAGES, Action.UPDATE)
  async updatePageStatus(@Req() req, @Param('id') id: string, @Body() dto: UpdatePageStatusDto) {
    return this.pagesService.updatePageStatus(id, dto, req.activeStore.id);
  }

  @Delete(':id')
  @Permissions(Resource.PAGES, Action.DELETE)
  async deletePage(@Req() req, @Param('id') id: string) {
    return this.pagesService.deletePage(id, req.activeStore.id);
  }
}
