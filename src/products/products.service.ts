...
export class ProductsService {
  ...
  async getProducts(storeId: string, status?: ProductStatus) {
    ...
    const stats = {
      ...
      active: await this.prisma.product.count({
        where: { storeId, status: 'ACTIVE' },
      }),
      outOfStock: await this.prisma.product.count({
        where: { storeId, quantity: 0 },
      }),
      drafts: await this.prisma.product.count({
    ...
    try {
      ...
      if (error.code === 'P2002') {
        throw new ConflictException(
          'Product slug already exists for this store',
        );
      }
      throw error;
    }
